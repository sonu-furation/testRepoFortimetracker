...
import { Employee } from "@data/employee/models/employee-models";
...
export class EmployeeService {
  ...
  async logOutEmployee(req: any, res: Response): Promise<void> {
    try {
      res
        .status(200)
        .cookie("token", null, {
  ...
  async resetPassord(req: any, res: any) {
    ...
    if (employees.isRightValue) {
      ...
      try {
        await sendEmail({
          email: employees.value.email,
          message,
        });
        res.status(200).json({
          success: true,
          message: `Email sent to ${employees.value.email}`,
        });
      } catch (error: any) {
        res.send(error);
      ...
      res.send(employees.value.message);
    }
  ...
  async verifyOtp(req:any,res:any):Promise<any>{
    ...
    if(userOtp == otp){
      res.send({msg:"Otp valid"})
    }else{
      res.send({msg:"invalid OTP"})
    }
  }
  async changePassWord(req:any,res:any):Promise<any>{
    const {userId,newPass} = req.body
    // console.log(req.body.userId)
    bcrypt.hash(newPass, 10,async function(err: any, hash: any) {
      // Store hash in your password DB.
      if(err){
        res.send({msg:"facing issue while hasing the password"})
      }else{
        const user = await Employee.findByIdAndUpdate({_id:userId},{password:hash})
        // const updatedPass = Employee.find({_id:userId})
        res.send(user)
      }
  });
    // const user = await Employee.findByIdAndUpdate({_id:userId},{password:newPass})
    // // const updatedPass = Employee.find({_id:userId})
    // res.send(user)
    // console.log(user)
  }
